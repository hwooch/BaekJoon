...
// 아이디어
// main에서 len의 변수를 통해 처음 입력한 숫자가 한자리라면 바로 결과를 나타내는 코드를 거친 후에
// 
// 만약 2자리 이상이라면 trans라는 함수를 통해 반복적인 변환을 거치게 됩니다.
//
// 그 변환의 횟수를 저장하는 변수가 time 변수이고 count는 자릿수를 나타내는 변수로,
//
// 1자리가 아니라면 while문을 통해 계속해서 변환을 시도합니다.
//
// trans의 함수에서 sum은 전체 각 자릿수의 숫자들을 모두 더해 저장하는 변수이고
//
// 만약 모두 더한 숫자가 10이 넘어간다면 각 자릿수를 쪼개 temp에 저장하는 과정을 거치게 됩니다.
//
// 그 뒤에 temp를 다시 user변수로 집어넣어 main의 trans함수에서 변환된 값을 이용할 수 있게끔 하였습니다

int trans(int user[], int temp[], int len) {
	int sum = 0;
	for (int i = 0; i < len; i++) {
		sum = sum + user[i];
	}
...
int main() {
	...
	// 문자열을 숫자로 변환하고 배열에 저장
	for (int i = 0; str[i] != '\0'; i++) {
		user[i] = str[i] - '0';
		len++;
	}
	if (len < 2) {
		if (user[0] == 3 || user[0] == 6 || user[0] == 9) {
			printf("0\n");
			printf("YES");
		}
		else {
	...
	while (count != 1) {
		count = trans(user, temp, len);
		len = count;
		time++;
	}

	if (temp[0] == 3 || temp[0] == 6 || temp[0] == 9) {
		printf("%d\n", time);
		printf("YES");
	}
	else {
