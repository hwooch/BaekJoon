...
// 
// 입력
// 첫째 줄에는 현재 시각이 나온다.현재 시각은 시 A(0 ≤ A ≤ 23),
// 분 B(0 ≤ B ≤ 59)와 초 C(0 ≤ C ≤ 59)가 정수로 빈칸을 사이에 두고 순서대로 주어진다.
...
int main() {
	...
	for (int i = 0; i < 3; i++) {
		scanf("%d", &user);
		time[i] = user;  // 시, 분, 초 입력
	}
	scanf("%d", &user); // 요리시간 입력

	time[2] = user % 60 + time[2];
	time[1] = user / 60 + time[1];
